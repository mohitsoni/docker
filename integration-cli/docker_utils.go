...
func deleteVolume(volume string) error {
	...
	cmd := exec.Command(dockerBinary, "volumes", "rm", volume)
...
func getAllVolumes() (string, error) {
	cmd := exec.Command(dockerBinary, "volumes", "ls", "-q")
...
func inspectVolumeField(name, field string) (string, error) {
	format := fmt.Sprintf("{{.%s}}", field)
	inspectCmd := exec.Command(dockerBinary, "volumes", "inspect", "-f", format, name)
	out, exitCode, err := runCommandWithOutput(inspectCmd)
	if err != nil || exitCode != 0 {
		return "", fmt.Errorf("failed to inspect %s: %s", name, out)
	}
